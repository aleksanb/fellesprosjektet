...
public class AlarmEventSupport {
	...
	public void fireAlarmEvent(Alarm alarm) {
		Appointment appointment = getAppointmentByAlarmId(alarm);
		if(appointment==null)
			System.out.println("no match between alarmId and appointmentId");
		for (AlarmListener listener : listeners) {
			listener.alarmEvent(appointment);
		}
